...
const AIMatchmaker: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col glass rounded-3xl overflow-hidden border border-white/10 shadow-2xl relative bg-[#0a0a0a]/80 backdrop-blur-xl">
      {/* 1. Header (Fixed at top of component) */}
      <div className="p-5 border-b border-white/5 bg-white/2 backdrop-blur-md shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-white tracking-tight">AI Matchmaker</h3>
              <p className="text-[10px] text-white/40 uppercase tracking-widest font-bold">24/7 Expert Support</p>
            </div>
          </div>
      ...
      {/* 2. Internal Feed Layout (Scrollable Area) */}
      <div 
        ref={scrollRef}
      ...
      >
        ...
        <div className="pt-4 flex flex-wrap gap-2">
          <p className="w-full text-[10px] text-white/30 uppercase tracking-widest font-bold mb-1">Strategic suggestions:</p>
      ...
      {/* 3. Pinned Input Area (Permanently docked at bottom) */}
      <div className="p-4 bg-white/2 border-t border-white/5 mt-auto backdrop-blur-md shrink-0">
        <div className="relative">
          <textarea 
            ...
            rows={1}
            className="w-full bg-white/5 border border-white/10 focus:border-purple-600/50 focus:ring-0 rounded-2xl py-4 pl-4 pr-12 text-sm text-white placeholder-white/20 transition-all resize-none min-h-[56px] max-h-32"
          />
          <button className={`absolute ${isRTL ? 'left-3' : 'right-3'} bottom-3 p-2.5 bg-gradient-to-br from-purple-600 to-blue-600 rounded-xl text-white hover:opacity-90 transition-all shadow-lg shadow-purple-600/20 active:scale-90`}>
            <Send className={`w-4 h-4 ${isRTL ? 'rotate-180' : ''}`} />
          </button>
        </div>
        <p className="mt-3 text-[10px] text-center text-white/30 font-bold uppercase tracking-widest">
          AI-Matched Cohort Orchestration 🎯
        </p>
      </div>
    </div>
  );
